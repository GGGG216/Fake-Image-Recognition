...
class FixedImageDownloader:
    def __init__(self):
        ...
        # 状态跟踪
        self.download_log = os.path.join(self.base_dir, "download_log_fixed.json")
        self.load_progress()
    ...
    def load_progress(self):
        ...
        if os.path.exists(self.download_log):
            ...
            self.progress = {
                'downloaded_files': [],  # 改为记录已下载的文件名
                'failed_prompts': [],
                'total_downloaded': 0,
                'start_time': datetime.now().isoformat(),
                'api_usage': {config['name']: 0 for config in self.api_configs},
                'fixed_range_start': 738,  # 固定从739开始
                'fixed_range_end': 741     # 到905结束
            }
            print("🆕 创建新的修复下载进度文件")
    ...
    def download_missing_images(self):
        ...
        start_index = self.progress['fixed_range_start'] - 1  # 739-1=738
        end_index = self.progress['fixed_range_end'] - 1      # 905-1=904
        
        print(f"🎯 开始修复下载缺失的图片")
        print(f"📊 目标范围: {start_index + 1}-{end_index + 1} (共{end_index - start_index}张)")
        ...
        for i in range(start_index, end_index + 1):
            ...
            if not prompt_str:
                print(f"⏭️ 跳过空提示词: 索引 {i+1}")
                continue
            
            # 生成预期的文件名
            clean_prompt = self.clean_filename(prompt_str)
            expected_filename = f"{i+1:04d}_{clean_prompt}.jpg"
            
            # 检查文件是否已存在
            if self.check_file_exists(expected_filename):
                print(f"⏭️ 文件已存在: {expected_filename}")
                if expected_filename not in self.progress['downloaded_files']:
                    self.progress['downloaded_files'].append(expected_filename)
                    self.progress['total_downloaded'] += 1
                    self.save_progress()
                continue
            
            print(f"\n🔍 下载缺失图片 ({i+1}/{end_index + 1}): '{prompt_str}'")
            missing_count += 1
            ...
            if photo:
                ...
                # 下载图片
                if self.download_image(image_url, expected_filename):
                    print(f"  ✅ 下载成功: {expected_filename}")
                    self.progress['downloaded_files'].append(expected_filename)
                    self.progress['total_downloaded'] += 1
                    success_count += 1
                else:
                    print(f"  ❌ 下载失败")
                    self.progress['failed_prompts'].append(f"{i+1}: {prompt_str}")
            else:
                print(f"  ⚠️ 未找到图片")
                self.progress['failed_prompts'].append(f"{i+1}: {prompt_str}")
            ...
            print(f"  📊 进度: 成功{success_count}/缺失{missing_count} | 可用API: {enabled_apis}")
        ...
        print(f"\n🎉 修复下载完成！")
        print(f"✅ 成功下载: {success_count} 张缺失图片")
...
def main():
    downloader = FixedImageDownloader()
    
    print("🚀 Unsplash缺失图片修复下载器")
    print("=" * 50)
    print(f"📁 工作目录: {os.getcwd()}")
    print(f"📁 图片保存到: {downloader.image_dir}")
    print(f"🎯 目标范围: 739-905号图片")
